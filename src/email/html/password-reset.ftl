...
<#assign
    ...
    expiration=msg("emails.reset.expiresIn", linkExpirationFormatter(linkExpiration))
    fallbackIntro=msg("emails.fallbackIntro")
    fallbackLabel=msg("emails.reset.fallbackLabel")
    securityNote=msg("emails.reset.security")
    disclaimer=msg("emails.reset.disclaimer")
